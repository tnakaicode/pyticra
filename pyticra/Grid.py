...
class Grid(Map):
    ...
    def load_grd(self, filename):
        ...
        with open(filename, 'r') as f:
            ...
            self.NSET, self.ICOMP, self.NCOMP, self.IGRID = [
        ...
        E_co = (data[:, 0] + 1j * data[:, 1]
                ).reshape(self.NX, self.NY, order='F')
        E_cx = (data[:, 2] + 1j * data[:, 3]
                ).reshape(self.NX, self.NY, order='F')
    ...
    def save_grd(self, filename):
        ...
        with open(filename, 'w') as f:
            ...
            for i in range(self.NX * self.NY):
                f.write(' {: 0.10E} {: 0.10E} {: 0.10E} {: 0.10E}\n'.format(
                    E_co[i].real, E_co[i].imag, E_cx[i].real, E_cx[i].imag))
